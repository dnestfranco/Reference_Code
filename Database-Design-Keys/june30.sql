...
-- Extract all pokemon data that weigh more than 500 grams, limit 10 and sort by descending order
SELECT `pok_name`
...
-- Extract all fire pokemon names heavier than 500 decigrams
SELECT `pok_name`
FROM `pokemon_db`.`pokemon`, `pokemon_db`.`types`
WHERE `pokemon`.`pok_id` = `types`.`pok_id` AND `pok_weight` > 500 AND `type1_name` = 'Fire';
